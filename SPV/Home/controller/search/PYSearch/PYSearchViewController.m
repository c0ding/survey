...
//    
//    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"fanhui"] style:UIBarButtonItemStyleDone target:self action:@selector(cancelDidClick)];
//    UIButton *rightBarBtn = [[UIButton alloc] initWithFrame:CGRectZero];
...
//    searchBar.py_width -= PYMargin * 1.5;
    ...
    [searchField setValue:getUIColor(0xA9A9A9) forKeyPath:@"_placeholderLabel.textColor"];
    [searchField setValue:[UIFont regulerApplicationFontOfSize:font(15)] forKeyPath:@"_placeholderLabel.font"];
...
- (void)setupSearchHistoryTags
{
    ...
    // 添加搜索历史头部
    self.searchHistoryHeader.py_y = self.hotSearches.count > 0 ? CGRectGetMaxY(self.hotSearchTagsContentView.frame) + kHeight(88) + PYMargin * 1.5 : 0;
