...
@interface UIView (Add)<UIScrollViewDelegate>

- (void)setCornerRadius:(CGFloat)radius withShadow:(BOOL)shadow withOpacity:(CGFloat)opacity andShadowColor:(UIColor *)shadowColor;
...
- (void)setCornerRadius:(CGFloat)radius withShadow:(BOOL)shadow withOpacity:(CGFloat)opacity andShadowColor:(UIColor *)shadowColor  {
    ...
    if (shadow) {
        self.layer.borderColor = RGBA(189, 192, 192,0.15).CGColor;
...
- (void)setAllowDropDown:(BOOL)allowDropDown {
...
- (void)layoutSubviews {
    ...
    [self.tableView setCornerRadius:6 withShadow:YES withOpacity:0.26 andShadowColor:RGB(189, 192, 192)];
}
...
- (void)deselectCell {
//    [self.tableVie th:self.tableView.indexPathForSelectedRow animated:NO];
}
