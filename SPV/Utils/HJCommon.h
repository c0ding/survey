...
#define dealloc_log_on

#ifdef dealloc_log_on
#define HJDeallocLog(format,...) NSLog(format,##__VA_ARGS__)
#else
#define HJDeallocLog(format,...)
#endif
...
/**
 *  通过时间秒数获得时间文字，时：分。 返回格式： 00:00
 *
 *  @param time 时间秒数
 *
 *  @return 时间文字
 */
extern NSString* timeFromTimeInterval2(NSTimeInterval time);
...
/*
 ...
 @return 返回字符串
*/
extern NSString *formatFloat(double f);
/**
 给定数字字符串(可以包含小数点)

 @param priceStr 金额数字字符串
 @return 返回插入分隔符的字符串
 */
extern NSString *addSeperatorPriceingSting(NSString *priceStr);
/**
