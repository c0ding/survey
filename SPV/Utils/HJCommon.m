...
NSString* timeFromTimeInterval2(NSTimeInterval time){
...
#pragma mark== iOS小数点格式化：如果有两位小数不为0则保留两位小数，如果有一位小数不为0则保留一位小数，否则显示整数
NSString *formatFloat(double f){
//    NSLog(@"====%f====%f",fmodf(f, 1),fmod(f*10, 1));
...
#pragma mark===如 23432432432.34 ==》23,432,432,432.34
NSString *addSeperatorPriceingSting(NSString *priceStr)
{
