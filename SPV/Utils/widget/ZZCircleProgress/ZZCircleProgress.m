...
- (void)drawRect:(CGRect)rect {
    ...
    if (_showProgressText) {
        ...
        if(_decimalNumber==0){
            currentText= [NSString stringWithFormat:@"%.0f%%",round(fakeProgress*100)];
        }else{
            //保留一位小数,如果是36.0则显示为36
            currentText=[NSString stringWithFormat:@"%@%%",formatFloat(round(fakeProgress*100*10)/10)];
        }
        //段落格式
        ...
        //字体
        UIFont *font = [UIFont systemFontOfSize:0.2*CircleSelfWidth];
        //构建属性集合
...
//    }
    //修改大于1也行
    if ((_progress == progress && !_forceRefresh) || progress<0.0) {
        return;
    }
    ...
    timer = [NSTimer scheduledTimerWithTimeInterval:0.005 block:^{
        ...
        if (isReverse) {
            if (fakeProgress <= _progress || fakeProgress <= 0.0f) {
                ...
                //进度条动画
                [strongSelf setNeedsDisplay];//不加动画
            }
        } else {
            //正方向动画
