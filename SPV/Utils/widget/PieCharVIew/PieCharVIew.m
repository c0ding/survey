...
- (void)drawRect:(CGRect)rect
{
    if (_dataArray) {
        ...
        for (PieCharModel *model in _dataArray) {
            ...
            drawArc(ctx, CGPointMake(self.frame.size.width/2.0, self.frame.size.height/2.0), radians(radius_start), radians(radius_start + radius_end), model.color, self.radius);
            if (_isShowTitle) {
                CGContextSetLineWidth(ctx, 1.0f);
                [model.color set];
                // 起始
                CGPoint point1 = [self getPostionX:self.frame.size.width/2.0 withPostionY:self.frame.size.height/2.0 withRadius:_radius - 20 withCirAngle:(radius_start + radius_end / 2)];
                ...
                // 延伸
                CGPoint point2 = [self getPostionX:self.frame.size.width/2.0 withPostionY:self.frame.size.height/2.0 withRadius:_radius + 10 withCirAngle:(radius_start + radius_end / 2)];
                // 折线
                CGFloat line_x;
                CGFloat label_x;
