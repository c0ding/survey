...
//
//  Created by HMW on 16/11/9.
//  Copyright © 2016年 HMW. All rights reserved.
//
...
- (void)setLeftViewController:(UIViewController *)leftViewController
{
    ...
    _leftContainerView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, -kZYDeviceWidth, 0);
    _leftContainerView.transform = CGAffineTransformScale(_leftContainerView.transform, 1, 1);

//    [_maskView setFrame:CGRectMake(20, 20, 80, 80)];
//    [_leftContainerView addSubview:_maskView];
...
- (UINavigationController *)sliderNavigationController
{
    if (self.mainViewController) {
        ...
        if ([self.leftViewController isKindOfClass:[UINavigationController class]]) {
            return (UINavigationController *)self.leftViewController;
        }
    }
    return nil;
}
            ...
            if (!_isLeftShow&&!_isRightShow) {
                ...
                switch (_dragDirection) {
                    case ZYDragDirectionLeft:
                    {
                        ...
                        if (move_length>kZYDeviceWidth||left_x>0||left_x+move_length>0) {
                            // 判断界面变化是否应该停止
                            break;
                        }
                        
                        scale = 1-(move_length/kZYDeviceWidth)*(1-leftScale);
...
//                        CGFloat left_scale = 1+(move_length/kZYDeviceWidth)*(1-leftScale);
                ...
                if (fabs(move_length)>kZYDeviceWidth||left_x>0||left_x+move_length>0||point.x>_startDragPoint.x||main_x<=0) {
                    break;
                }
...
- (void)showLeft
{
    ...
    [UIView animateWithDuration:[self calculateAnimationDurationIsShow:YES]
                     animations:^{
                         ...
                         _mainContainerView.transform = CGAffineTransformTranslate(CGAffineTransformIdentity, kZYDeviceWidth, 0);
                         _mainContainerView.transform = CGAffineTransformScale(_mainContainerView.transform, 1, 1);
